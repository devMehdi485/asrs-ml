...
\newcommand{\exemple}[1]{\par\smallskip\noindent\colorbox{boxbg}{\parbox{\dimexpr\linewidth-2\fboxsep}{%
  \small\textbf{\color{acc}Application aux données ASRS.} #1}}\par\smallskip}
...
\centering
\vspace*{1.3cm}
{\small\color{acc}\textbf{PROJET INFORMATIQUE -- NLP ET SÉCURITÉ AÉRIENNE}}\\[0.9cm]
...
{\small Projection des 125\,763 rapports ASRS dans l'espace sémantique}
\vfill
\begin{tabular}{rl}
...
\textbf{Streamlit} (Python) et une application web \textbf{React} (« AeroInsight AI »). Cette
dernière comporte sept pages : une vue d'ensemble (volumes, répartition par catégorie), un
explorateur de clusters (carte sémantique colorée par catégorie, sélection d'un thème, nuage
de mots et récits représentatifs), une analyse thématique (importance et tendance de chaque
thème), une carte temporelle, une page de signaux faibles, une page de prédiction (le
classifieur s'exécute directement dans le navigateur sur un récit saisi) et une synthèse
décisionnelle. L'objectif est de rendre les résultats explorables y compris par un lecteur
non spécialiste.
